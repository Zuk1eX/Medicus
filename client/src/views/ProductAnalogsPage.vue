<template>
    ...
    <div class="container section-results">
        <div class="results__top">
            <h3 class="results__title">{{ productPharmgroup || productActionResultsProperty.pharmgroup }}</h3>
    ...
    <div v-intersection="loadMoreAnalogs" class="observer" v-if="productsCount && !loadingState"></div>
</template>
<script>
...
export default {
    ...
    methods: {
        ...
        ...mapActions(["getProductAnalogsAPI", "getMoreProductAnalogsAPI"]),
        getProductAnalogs() {
            this.changeLoadingState(true);
            setTimeout(() => {
                this.getProductAnalogsAPI([
                    this.productId,
                    {
                        limit: this.limit,
    ...
    computed: {
        ...
        productPharmgroup() {
            return this.$route.params.pharmgroup ?? "";
        },
        productsCount() {
    ...
    watch: {
        $route(value) {
            if (this.$route.name === "productForms" && value.params.id) {
                ...
                this.currentPage = 1;
                this.getProductAnalogs();
            }
        },
    },
    mounted() {
        if (this.productDataEmpty || this.productPharmgroup !== this.productActionResultsProperty.pharmgroup) {
            this.currentPage = 1;
            this.getProductAnalogs();
        }
    },
